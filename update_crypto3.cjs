...
export async function getCryptoRates(): Promise<CryptoRates | null> {
  ...
  // 3. Binance (Tertiary) - Fetch remaining missing tickers
  const missingBinance = targetTickers.filter(t => !rates[t] && TICKER_TO_BINANCE[t]);
  if (missingBinance.length > 0) {
    try {
      const symbols = missingBinance.map(t => TICKER_TO_BINANCE[t]).filter(s => s !== 'USDTUSDT');
      if (symbols.length > 0) {
      ...
      console.error('Binance fallback error:', err);
    }
  }
...
console.log('crypto.ts updated with robust merging logic');
