...
{assetTabOrder.map((tab) => {
              if (hiddenTabs.includes(tab)) return null;
              const meta = {
                cash: { label: 'Сейф', Icon: Vault },
                investments: { label: 'Биржа', Icon: ChartNoAxesCombined },
                crypto: { label: 'Крипта', Icon: Bitcoin },
              }[tab];
              return (
                <button
                  key={tab}
                ...
                >
                  <span className="relative z-20 flex items-center gap-2">
                    <meta.Icon size={13} className={cn("transition-colors", activeTab === tab ? "opacity-100" : "opacity-60")} />
                    <span className="uppercase tracking-widest truncate">{meta.label}</span>
                  </span>
                  {activeTab === tab && (
                    <motion.div 
                      layoutId="activeAssetsTabPill"
