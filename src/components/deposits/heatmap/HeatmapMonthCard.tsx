...
export function HeatmapMonthCard({ month, heatmapData, setExpandedMonth }: HeatmapMonthCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-white/60 dark:bg-slate-950/60 backdrop-blur-md rounded-[1rem] sm:rounded-[1.25rem] border border-slate-200/60 dark:border-white/[0.05] p-1.5 sm:p-2 lg:p-2.5 xl:p-3 pb-1.5 sm:pb-2 flex flex-col cursor-pointer hover:border-deposit-500/40 dark:hover:border-deposit-500/30 hover:shadow-lg hover:-translate-y-0.5 transition-all group relative overflow-hidden"
    >
      ...
      {/* Mini heat grid */}
      <div className="grid grid-cols-7 grid-rows-6 gap-[1.5px] sm:gap-[2px] lg:gap-[2.5px] mt-auto">
        {emptyDays.map((_, i) => (
