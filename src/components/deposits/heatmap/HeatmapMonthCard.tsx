...
export function HeatmapMonthCard({ month, heatmapData, setExpandedMonth }: HeatmapMonthCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Mini heat grid */}
      <div className="grid grid-cols-7 grid-rows-6 gap-[2px] sm:gap-[3px] lg:gap-[3px] mt-auto">
        {emptyDays.map((_, i) => (
        ...
        {days.map(day => {
            ...
            return (
              ...
              >
                <span className={cn(
                  "absolute inset-0 flex items-center justify-center text-[8px] sm:text-[9px] lg:text-[9px] xl:text-[12px] font-black leading-none pointer-events-none",
                  intensity === 0 ? "text-slate-400/60 dark:text-slate-500/60" : "text-white/90",
                  isMaturing && "text-amber-500 dark:text-amber-400/90 lg:text-[8px] drop-shadow-sm"
                )}>
                  {format(day, 'd')}
                </span>
