...
export function HeatmapMonthCard({ month, heatmapData, setExpandedMonth }: HeatmapMonthCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-white/60 dark:bg-slate-950/60 backdrop-blur-md rounded-[1rem] sm:rounded-[1.25rem] border border-slate-200/60 dark:border-white/[0.05] p-2 sm:p-2.5 md:p-3 pb-2 flex flex-col cursor-pointer hover:border-deposit-500/40 dark:hover:border-deposit-500/30 hover:shadow-lg hover:-translate-y-0.5 transition-all group relative overflow-hidden"
    >
      ...
      {/* Mini heat grid */}
      <div className="grid grid-cols-7 grid-rows-6 gap-[2px] sm:gap-[3px] lg:gap-[4px] mt-auto">
        {emptyDays.map((_, i) => (
          <div key={`empty-${i}`} className="aspect-square rounded-[3px] sm:rounded-[4px]" />
        ))}
        {days.map(day => {
            ...
            return (
              <div 
                key={dateKey} 
                className={cn(
                  "aspect-square rounded-[2px] sm:rounded-[3px] lg:rounded-[4px] relative flex items-center justify-center pointer-events-none",
                  getColorClass(intensity),
                  isMaturing && "ring-[1px] sm:ring-[1.5px] ring-amber-400 dark:ring-amber-500 ring-inset shadow-[0_0_8px_rgba(251,191,36,0.5)] z-10",
                  intensity > 0 ? "border border-white/10 shadow-sm" : ""
                )}
              >
                <span className={cn(
                  "text-[7px] sm:text-[8px] lg:text-[9px] xl:text-[11px] font-black leading-none pointer-events-none mt-[1px]",
                  intensity === 0 ? "text-slate-400/60 dark:text-slate-500/60" : "text-white/90",
                  isMaturing && "text-amber-600 dark:text-amber-400/90 drop-shadow-sm"
                )}>
                  {format(day, 'd')}
                </span>

                {isOpening && (
                  <div className="absolute -top-1 -right-1 z-10 drop-shadow-[0_0_2px_rgba(var(--rgb-deposit),0.8)] flex items-center justify-center">
