  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white dark:bg-slate-900 rounded-t-[32px] sm:rounded-[32px] shadow-2xl max-w-sm w-full p-6 sm:p-8 animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-300 border border-slate-200 dark:border-slate-800">
        ...
        <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 tracking-tight">Удалить вклад?</h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-8 leading-relaxed">
          Вы уверены, что хотите удалить вклад в банке <strong className="text-slate-900 dark:text-white">{deposit.bank}</strong>? Восстановить запись можно будет в архиве.
        </p>
        <div className="flex items-center gap-3">
          <button
            onClick={onCancel}
            className="flex-1 apple-button bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white hover:bg-[#E5E5E7] dark:hover:bg-white/10 text-sm sm:text-base"
          >
            Отмена
          </button>
          <button
