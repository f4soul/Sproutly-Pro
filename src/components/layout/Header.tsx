...
import { User, signInWithGoogle, logout } from '../../config/firebase';
...
export const Header = ({
  ...
  return (
    <header className="flex justify-between items-center apple-card p-4 relative z-50">
      ...
      <div className={cn(
        "absolute top-full right-0 mt-2 w-56 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-[24px] shadow-2xl border border-slate-200 dark:border-slate-800 p-2 flex-col gap-1 z-50 md:static md:w-auto md:bg-transparent md:dark:bg-transparent md:shadow-none md:border-none md:p-0 md:flex-row md:flex md:gap-3 transition-all duration-200 origin-top-right",
        isMobileMenuOpen 
            ...
            <AnimatePresence>
              {isProfileMenuOpen && (
                <motion.div 
                  ...
                  className="absolute right-0 mt-2 w-48 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-[24px] shadow-xl border border-slate-200 dark:border-slate-800 py-2 z-50 overflow-hidden hidden md:block"
                >
                  <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-800 mb-1">
                    <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Аккаунт</p>
        ...
        <button 
          ...
          className="w-full md:w-10 h-10 flex items-center justify-start md:justify-center gap-3 px-3 md:px-0 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 md:bg-white md:dark:bg-slate-900 md:border md:border-slate-200 md:dark:border-slate-800 md:hover:border-blue-400 md:dark:hover:border-blue-500 transition-all cursor-pointer text-slate-900 dark:text-white md:text-slate-500 md:dark:text-slate-400 shadow-none md:shadow-sm" 
