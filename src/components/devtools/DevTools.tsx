        ...
        for (const record of testRecords) {
          if (record.id) {
            ...
            try {
              ...
              console.error('Failed to clear test data from Firebase', e);
            }
          }
        }
      }
      ...
      <div className="apple-card p-3 flex flex-col gap-2">
        <div className="flex items-center justify-between px-2 pb-2 border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-slate-900 dark:text-white">Dev Tools</span>
          </div>
          <button 
            onClick={() => setIsOpen(false)}
            className="p-1 hover:bg-slate-50 dark:hover:bg-white/10 rounded-lg transition-colors cursor-pointer text-slate-500 hover:text-slate-900 dark:hover:text-white"
        ...
        <button 
          onClick={addTestData}
          className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 dark:hover:bg-white/10 rounded-xl transition-colors text-xs font-bold cursor-pointer text-slate-900 dark:text-white"
        >
