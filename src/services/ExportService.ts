  ...
  try {
    ...
    logo.innerHTML = `
      <div style="display: flex; align-items: center; gap: 8px;">
        ...
        <span style="font-weight: 900; font-size: 24px; color: ${textColor}; font-family: 'JetBrains Mono', 'Inter', system-ui, sans-serif; letter-spacing: -2px; text-transform: uppercase;">
    ...
    if (incomeData) {
      ...
      thead.innerHTML = `
        <tr style="border-bottom: 2px solid ${borderColor}; text-align: left;">
          <th style="padding: 12px 8px;">Месяц</th>
          ...
          <th style="padding: 12px 8px;">На руки (Net)</th>
        </tr>
      `;
      ...
      incomeData.months.forEach((m, index) => {
        ...
        row.innerHTML = `
          <td style="padding: 12px 8px; font-weight: 700;">${monthNames[index] || ''}</td>
          ...
          <td style="padding: 12px 8px; color: #3b82f6; font-weight: 800;">${(m.net13 || 0).toLocaleString('ru-RU')} ₽</td>
        `;
    ...
    if (deposits && deposits.length > 0) {
      ...
      deposits.forEach(d => {
        ...
        row.innerHTML = `
          ...
          <td style="padding: 12px 8px;">${isDepositClosed(d) ? 'Закрыт' : 'Активен'}</td>
        `;
        tbody.appendChild(row);
      });
      table.appendChild(tbody);
...
export const exportToImage = async (elementId: string, deposits?: Deposit[]) => {
  ...
  try {
    ...
    const titleContainer = document.createElement('div');
    const title = document.createElement('h1');
    title.innerText = deposits && deposits.length > 0 ? 'ОТЧЕТ: ВКЛАДЫ' : 'ФИНАНСОВЫЙ ОТЧЕТ';
    title.style.fontSize = '28px';
    title.style.fontWeight = '900';
    title.style.letterSpacing = '-1px';
    title.style.margin = '0';
